...
\begin{itemize}
  ...
  \item \texttt{responseCode} is the HTTP response code that should be 
        ...
        which indicates that the \texttt{responseCode} has not been set yet. 
...
of an array.''\cite{tour}} of bytes or return an error in case they 
are provided with a 
...
back to the client using the \texttt{WriteError}\footnote{See Appendix for the 
implementation.} function. Otherwise, it 
