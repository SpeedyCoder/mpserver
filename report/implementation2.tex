...
behaves as follows:
\begin{itemize}
	\item It passes every input \texttt{job} to the worker, gets the result 
...
In order to implements a proxy server one might want to use
...
updates the request made by the client with the given scheme and path
in each input \texttt{job}. It then stores the request in the result 
field of the given \texttt{job} and outputs it.
...
taken from the Result field of the \texttt{job}, using the provided http client.
